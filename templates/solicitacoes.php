...
<div class="row-fluid">
	...
	<div class="span9">
		...
		<table width="100%" class="table table-striped">
			<thead>
				<tr>
					...
					<th width="80" class="hidden-phone">Status</th>
				</tr>
			</thead>
			<tbody>
			<?php 
			...
			?>
				...
				<tr>
					...
					<td><?= date('d/m/Y',$s->data->sec) ?><div class="visible-phone">
						<div class="status_<?= $s->status[1] ?>"><?= $verbal_status[$s->status[0]] ?></div>
					</div></td>
		...
		<?php if($pages > 1){ 
			$is_first = $page == 1;
			$is_last = $page == $pages;
			$first_url = $is_first ? '#' : '?p=' . ($page - 1) . (isset($_GET['page_size']) ? "&page_size=" . $_GET['page_size'] : "");
			$last_url = $is_last ? '#' : '?p=' . ($page + 1) . (isset($_GET['page_size']) ? "&page_size=" . $_GET['page_size'] : "");
		?>
